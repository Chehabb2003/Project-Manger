...
// multiLimiter implements a simple per-key token bucket limiter with cleanup.
...
// allow returns true if the key is permitted to proceed.
...
// getClientIP returns best-effort client IP (checks X-Forwarded-For first element, then RemoteAddr).
